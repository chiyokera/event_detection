...
class ActionSpotDataset(Dataset):
    ...
    def _store_clips(self):
        ...
        for video in tqdm(self._labels):
            # video = {video: england-efl/2019-2020/01-01-2020/game, num_frames: xxx}
            ...
            if self._dataset == "soccernet":
                ...
                labels_file = load_json(
                    os.path.join(
                        LABELS_SNB_PATH, video["video"] + "/Labels-ball-location.json"
                    )
                )["annotations"]
            else:
...
class ActionSpotVideoDataset(Dataset):
    ...
    def __init__(
        ...
        self._clips = []  # video name, frame_clip_start_idx
        for (
            l
        ) in (
            self._labels
        ):  # l = {video: england-efl/2019-2020/01-01-2020/game, num_frames: xxx}
            has_clip = False
            for i in range(
            ...
            for x in self._labels:
                ...
                if self._dataset == "soccernet":
                    ...
                    labels_file = load_json(
                        os.path.join(
                            LABELS_SNB_PATH,
                            x_copy["video"] + "/Labels-ball-location.json",
                        )
                    )["annotations"]
                    for e in labels_file:
                        e["frame"] = (
...
class FrameReaderVideo:
    ...
    def load_frames(
        ...
        for frame_num in range(start, end, stride):  # すべてのフレームを読み込む

            if frame_num < 0:
                n_pad_start += 1
                continue

            if "720p-frames" in str(self._frame_dir):
                frame_path = os.path.join(
                    self._frame_dir, "frame" + str(frame_num) + ".jpg"
                )
            else:
                frame_path = os.path.join(
            ...
            try:
                ...
                ret.append(img)
            except RuntimeError:  # If the image is corrupted
                n_pad_end += 1
...
class ActionSpotDatasetJoint(
    Dataset
):  # Joint dataset SoccerNet(for pretrain) and SoccerNet-Ball
...
class InferenceActionSpotVideoDataset(Dataset):
