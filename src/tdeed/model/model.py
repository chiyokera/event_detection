...
class TDEEDModel(BaseRGBModel):
    ...
    class Impl(nn.Module):
        ...
        def __init__(self, args=None):
            ...
            assert self._temp_arch in [
                "ed_sgp_mixer"
            ], "Only ed_sgp_mixer supported for now"
    ...
    def epoch(
        ...
        with torch.no_grad() if optimizer is None else nullcontext():
            for batch_idx, batch in enumerate(tqdm(loader)):
                ...
                if self._model._double_head:
                    ...
                    label = update_labels_2heads(
                        label, batch_dataset, self._args.num_classes
                    )
                ...
                if valMAP:
                    labels_aux = process_labels(
                        label,
                        labelD if "labelD" in batch.keys() else None,
                        ...
                        for i in range(pred.shape[0]):
                            if batch_dataset[i] == 1:
                                ...
                                loss += F.cross_entropy(
                                    pred[i][:, : self._args.num_classes + 1],
                                    aux_label,
                                ...
                                loss += F.cross_entropy(
                                    ...
                                    aux_label,
                                    weight=ce_kwargs["weight"][
                                        : self._args.pretrain["num_classes"] + 1
                                    ],
                                ) / (pred.shape[0])
        ...
        if valMAP:
            return (
                ...
                torch.cat(map_labels, 0),
                torch.cat(map_preds, 0),
            )
        ...
        with torch.no_grad():
            ...
            if isinstance(pred, dict):
                ...
                if isinstance(predD, list):
                    predD = predD[0]
                if self._model._double_head:
                    pred = process_double_head(
                        pred, predD, num_classes=self._args.num_classes + 1
                    )
                else:
