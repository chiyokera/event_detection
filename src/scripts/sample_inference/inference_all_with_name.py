...
def predict_fold(fold, gpu_id: int):
    ...
    for experiment, target in zip(experiments, targets):
        ...
        experiment_dir = Path(
            "/home/ilabnas3/tanaka/archive/event_detection/data/team_location_detection/experiments"
            + f"/{experiment}"
...
def get_name(df_game, frame_num, range_list, team_name):
    ...
    for i in range_list:
        ...
        df_frame = df_frame[df_frame["uclid"] != "#NUM!"]
        dic = {}

        if not df_frame.empty:
            df_last = df_frame[df_frame["uclid"] == df_frame["uclid"].min()]
            dic["name"] = "No_name"
            df_name = df_last["short_name"].to_string(index=False)
            if df_name != "NaN":
                dic["name"] = df_name
                df_name = df_name.replace(" ", "")
                df_name = df_name.replace("NaN", "")
        else:
            dic["name"] = "No_name"
...
def goal_replacement(results_path: str, gsr: bool = False):
        ...
        if result["action"] == "GOAL":
            if not i == len(results["predictions"]) - 1:
                ...
                if next_action == "BALL PLAYER BLOCK":
                    ...
                    if gsr:
                        results["predictions"][i]["name"] = results["predictions"][
                            i + 1
                        ]["name"]
                    ...
                    if gsr:
                        results["predictions"][i + 1]["name"] = results_i_dict["name"]
...
if __name__ == "__main__":
    ...
    if args.gsr:
        csv_path = os.path.join(
            ...
            "results",
            "video_info",
            "players_in_frames_sn_gamestate_noname.csv",
        )
        df = pd.read_csv(csv_path)
        df["name"] = df["name"].replace([None, ""], "No_name")
        df["uclid"] = df["uclid"].replace([None, ""], "#NUM!")
    ...
    for match_info in match_infos:
        ...
        for i, (action_pred, easy_pred, hard_pred, team_pred) in enumerate(
            ...
            if pred_dict["action"] == "BALL PLAYER BLOCK":
                ...
                if previous_team_own_side == "right":
                    ...
                    pred_dict["team"] = match_info["left"]
                elif previous_team_own_side == "left":
                    pred_dict["team_own_side"] = "right"
                    pred_dict["team"] = match_info["right"]
            ...
            if args.gsr:  # teamは求まっているので、nameのみ
                ...
                if action_pred["action"] == "OUT":
                    ...
                    if action_pred["action"] == "PASS":
                        range_list = range(-15, 2)
                    elif action_pred["action"] == "DRIVE":
                        range_list = range(-5, 15)
                    else:
                        range_list = range(-5, 10)
                    df_game = df[df["game"] == match_info["game"]]
                    df_game = df_game[df_game["half"] == match_info["half"]]
                    frame_info = get_name(
                        df_game, frame_num, range_list, pred_dict["team"]
                    )
        ...
        goal_replacement(save_path, gsr=args.gsr)
