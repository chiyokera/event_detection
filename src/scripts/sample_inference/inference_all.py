...
def predict_fold(fold, gpu_id: int):
    ...
    for experiment, target in zip(experiments, targets):
        ...
        video_info_path = os.path.join(
            DATA_DIR, "results", "video_info", "sample_video_info.json"
        )
        ...
        for video_info in video_infos:
            ...
            prediction_dir = os.path.join(
                DATA_DIR, "results", video_info["video"], experiment.replace("ball_tuning_", "")
            )
        ...
        if result["action"] == "GOAL":
            if not i==len(results["predictions"])-1:
                ...
                if next_action == "BALL PLAYER BLOCK":
                    ...
                    results["predictions"][i]["team"] = results["predictions"][i+1]["team"]
                    results["predictions"][i]["team_own_side"] = results["predictions"][i+1]["team_own_side"]
                    results["predictions"][i]["location"] = results["predictions"][i+1]["location"]
                    results["predictions"][i]["action_confidence"] = results["predictions"][i+1]["action_confidence"]
                    results["predictions"][i]["team_confidence"] = results["predictions"][i+1]["team_confidence"]
                    results["predictions"][i]["easy_confidence"] = results["predictions"][i+1]["easy_confidence"]
                    results["predictions"][i]["hard_confidence"] = results["predictions"][i+1]["hard_confidence"]
                    results["predictions"][i+1]["action"] = "GOAL"
                    results["predictions"][i+1]["team"] = results["predictions"][i]["team"]
                    results["predictions"][i+1]["team_own_side"] = results["predictions"][i]["team_own_side"]
                    results["predictions"][i+1]["location"] = results["predictions"][i]["location"]
                    results["predictions"][i+1]["action_confidence"] = results["predictions"][i]["action_confidence"]
                    results["predictions"][i+1]["team_confidence"] = results["predictions"][i]["team_confidence"]
                    results["predictions"][i+1]["easy_confidence"] = results["predictions"][i]["easy_confidence"]
                    results["predictions"][i+1]["hard_confidence"] = results["predictions"][i]["hard_confidence"]
...
if __name__ == "__main__":
    ...
    for match_info in match_infos:
        ...
        action_path = os.path.join(game_dir, "action", "results_spotting_my_filtered_action.json")
        ...
        for i, (action_pred, easy_pred, hard_pred, team_pred) in enumerate(zip(
