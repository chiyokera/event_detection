...
class GeneralizedMeanPooling(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.nn.functional.adaptive_avg_pool2d(x, self.output_size).pow(
            1.0 / self.p
        )
...
class InvertedResidual3d(nn.Module):
    def __init__(
        ...
        self.se = SqueezeExcite(
            mid_features, act_layer=act_layer, reduce_ratio=se_reduce_ratio
        )
...
class MultiDimStacker(nn.Module):
    """
    headを変えるときは、config finleのnum_classesを変更すること
    """
