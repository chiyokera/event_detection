...
from argus.utils import deep_chunk, deep_detach, deep_to

from team_location_detection.losses import FocalLoss
from team_location_detection.mixup import TimmMixup
from team_location_detection.models.multidim_stacker import MultiDimStacker
from torch import nn


class BallActionModel(argus.Model):
    nn_module = {"timm": timm.create_model, "multidim_stacker": MultiDimStacker}
    loss = {**pytorch_losses, "focal_loss": FocalLoss}
    prediction_transform = nn.Sigmoid

    def __init__(self, params: dict):
        self.freeze_conv2d_encoder = (
            False
            if "freeze_conv2d_encoder" not in params
    ...
    def train_step(self, batch, state: State) -> dict:
        ...
        return {"prediction": prediction, "target": target, "loss": loss_value}
    ...
    def val_step(self, batch, state: State) -> dict:
        ...
        with torch.no_grad():
            ...
            loss = self.loss(prediction, target)
            prediction = self.prediction_transform(prediction)
            return {"prediction": prediction, "target": target, "loss": loss.item()}
