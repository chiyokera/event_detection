...
class RandomCameraMove(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.param_generator = random_generator.AffineGenerator(
    ...
    def forward(self, frames_batch):
        ...
        for index, frames in enumerate(frames_batch):
            ...
            translations = tensor_linspace(
                params["translations"][0], params["translations"][1], num_frames
            ).T
            center = tensor_linspace(
                params["center"][0], params["center"][1], num_frames
            ).T
            scale = tensor_linspace(
                params["scale"][0], params["scale"][1], num_frames
            ).T
