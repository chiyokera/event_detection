...
def evaluate(
    ...
    if EVENT_DICTIONARY is None:
        if dataset == "SoccerNet" and version == 1 and task == "spotting":
            ...
            EVENT_DICTIONARY = {
                "1. Purposeful header": 0,
                "2. Header Duel": 1,
                "3. Attempted header": 2,
                "4. Unintentional header": 3,
                "5. Other head impacts": 4,
            }
        elif dataset == "Ball":
...
def label2vector(labels, num_classes=17, framerate=2, version=2, EVENT_DICTIONARY={}):
    ...
    for annotation in labels["annotations"]:
        ...
        if num_classes == 12:
            ...
            event = annotation["location"]
        elif num_classes == 2:
            event = annotation["team"]
        else:
            raise ValueError("num_classes should be 12, 17 or 2")
...
def predictions2vector(
    ...
    for annotation in predictions["predictions"]:
        ...
        if num_classes == 12:
            ...
            raise ValueError("num_classes should be 13, 17 or 2")
        ...
        if num_classes == 12:
            value = annotation["action_confidence"]
        else:
...
def average_mAP(
    ...
    INVERSE_EVENT_DICTIONARY = {v: k for k, v in EVENT_DICTIONARY.items()}
    for c in np.arange(len(mAP_per_class[0])):
        ...
        a_mAP_per_class[INVERSE_EVENT_DICTIONARY[c]] = integral_per_class / (
            (len(mAP_per_class) - 1)
        )
