...
from team_location_detection.augmentations import RandomCameraMove


def get_train_augmentations(size: tuple[int, int], target="label") -> nn.Module:
    size = size[::-1]
    ratio = size[0] / size[1]
    ...
    if target == "location_easy" or target == "location" or target == "team":
        transforms = nn.Sequential(
            ...
            augm.RandomResizedCrop(
                size, scale=(0.9, 1.0), ratio=(ratio - 0.1, ratio + 0.1), p=0.8
            ),
            augm.RandomSharpness(sharpness=1.0, p=0.2),
            ...
            augm.RandomGaussianNoise(mean=0.0, std=0.05, p=0.2),
        )
    else:
        transforms = nn.Sequential(
            RandomCameraMove((-2.5, 2.5), (0.1, 0.05), (0.95, 1.05), p=0.2),
